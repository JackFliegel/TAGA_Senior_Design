    ...
    private val scanSettings: ScanSettings
        get() {
            return if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                appendLog("here:"+ Build.VERSION_CODES.M)
                scanSettingsSinceM
            } else {
                scanSettingsBeforeM
            }
...
//        .setScanMode(ScanSettings.SCAN_MODE_LOW_LATENCY) // More aggressive scanning
        .setCallbackType(ScanSettings.CALLBACK_TYPE_ALL_MATCHES) // Callback for all matches
        .setMatchMode(ScanSettings.MATCH_MODE_AGGRESSIVE)
        .setNumOfMatches(ScanSettings.MATCH_NUM_MAX_ADVERTISEMENT) // Consider more advertisements
        .setReportDelay(0)
        .build();
